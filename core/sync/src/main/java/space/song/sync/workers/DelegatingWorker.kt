 ...
 * delegate to
 */
internal fun KClass<out CoroutineWorker>.delegatedData(dataTypes: List<SyncableType>? = null) =
    Data.Builder()
        .putString(WORKER_CLASS_NAME, qualifiedName)
        .apply {
            dataTypes?.let {
                putStringArray(
                    INPUT_DATA_SYNC_TYPES_KEY,
                    it.map { syncableType -> syncableType.name }.toTypedArray(),
                )
            }
        }
        .build()
